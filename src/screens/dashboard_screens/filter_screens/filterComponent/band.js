...
export default class BandFilter extends Component {
  constructor(props) {
    super(props)
    this.state = {
      showLoader:true,
  ...
  goBackDashboard(){
    dataFilterBand(this.state.selectedData);          
    setTimeout(() => { 
            ...
            <View>
              <ListView
                ...
                renderRow={(item) =>
                  <ListItem>
                    <CheckBox
                      checked={this.state.selectedData.includes(item.V_BAND_POSISI) ? true : false}
                      onPress={()=>this.onCheckBoxPress(item.V_BAND_POSISI)}
                    />
                    <Body>
