...
export default class FUFilter extends Component {
  constructor(props) {
    super(props)
    this.state = {
      showLoader:true,
  ...
  goBackDashboard(){
    dataFilterFU(this.state.selectedData);          
    setTimeout(() => { 
            ...
            <View>
              <ListView
                ...
                renderRow={(item) =>
                  <ListItem>
                    <CheckBox
                      checked={this.state.selectedData.includes(item.C_FUNCTION_UNIT) ? true : false}
                      onPress={()=>this.onCheckBoxPress(item.C_FUNCTION_UNIT)}
                    />
                    <Body>
