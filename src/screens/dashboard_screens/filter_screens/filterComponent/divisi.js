...
export default class DivisiFilter extends Component {
  constructor(props) {
    super(props)
    this.state = {
      showLoader:true,
  ...
  goBackDashboard(){
    dataFilterDivisi(this.state.selectedData);          
    setTimeout(() => { 
            ...
            <View>
              <ListView
                ...
                renderRow={(item) =>
                  <ListItem>
                    <CheckBox
                      checked={this.state.selectedData.includes(item.C_KODE_DIVISI) ? true : false}
                      onPress={()=>this.onCheckBoxPress(item.C_KODE_DIVISI)}
                    />
                    <Body>
